...
令牌当场校验，填错或已禁用会立刻报错。如果一个原本可用的令牌之后被禁用，HA 会弹出重新认证提示，换个新令牌即可，**历史数据不会丢**。
...
- 停止上报的设备会被判为离线，实体变成**不可用**。网关连不上或令牌被禁用时，全部实体同样变为不可用 —— 不会停在最后一次的读数上装作还在更新。
...
| 提示令牌无效或已禁用 | 令牌被禁用或填错了。HA 会弹出重新认证，换新令牌即可 |
